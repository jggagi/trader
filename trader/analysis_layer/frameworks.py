...
def _answer_cycle_position(move_pct: float | None, trend: str, risk_label: str) -> str:
    if move_pct is None:
        return "Dalio 式回答：价格和新闻证据不足，先不要给周期下结论。"
    if risk_label == "波动偏高":
        return "Dalio 式回答：这更像风险偏好下降或宏观变量扰动阶段，先看组合脆弱性而不是预测单点方向。"
    if move_pct >= 5 and trend in {"上升趋势", "短线偏强"}:
        return "Dalio 式回答：当前像风险资产顺风期，但还要验证是盈利改善、政策宽松，还是单纯估值扩张。"
    if move_pct <= -5:
        return "Dalio 式回答：这像风险资产逆风期，要拆成增长、通胀、利率、信用四个驱动分别看。"
    return "Dalio 式回答：周期信号不极端，最佳动作通常是观察、再平衡，而不是大幅押注。"


def _answer_risk_balance(risk_label: str, max_drawdown: float | None) -> str:
    drawdown = _format_optional_pct(max_drawdown)
    if risk_label == "波动偏高":
        return f"Dalio 式回答：最大回撤 {drawdown}，说明它可能正在放大组合波动，需要检查是否有现金、债券、红利或其他区域资产对冲。"
    if risk_label == "中等波动":
        return f"Dalio 式回答：最大回撤 {drawdown}，风险还可观察，但不能假设相关性永远分散。"
    return f"Dalio 式回答：最大回撤 {drawdown}，短期风险温和，但真正的风险平衡要看压力情景下的相关性。"


def _answer_policy_sensitivity(snapshot: dict) -> str:
    style = str(snapshot.get("selected_preset_style") or "")
    theme = str(snapshot.get("selected_preset_theme") or "")
    if style in {"科技成长", "成长"} or "科技" in theme:
        return "Dalio 式回答：成长和科技暴露通常对利率与流动性更敏感，折现率上行会压估值，流动性宽松会放大弹性。"
    if style == "低波红利" or "红利" in theme:
        return "Dalio 式回答：红利/低波更像防守资产，但仍会受利率替代效应和经济下行盈利压力影响。"
    return "Dalio 式回答：先判断它主要暴露于增长、通胀、利率还是货币，再决定需要什么对冲。"


def _answer_diversification(snapshot: dict) -> str:
    market = str(snapshot.get("market") or "Unknown")
    style = str(snapshot.get("selected_preset_style") or "")
    if style == "大盘核心":
        return f"Dalio 式回答：这是 {market} 的核心 beta，适合作为组合底仓，但不能替代跨国家、货币和资产类别的分散。"
    if style:
        return f"Dalio 式回答：这是 {market} 的 {style} 暴露，可能增加风格集中度，要看组合里是否已经有类似因子。"
    return f"Dalio 式回答：这是 {market} 标的，至少要检查国家、行业、货币和因子是否已经过度集中。"


def _answer_circle_of_competence(snapshot: dict) -> str:
    ticker = snapshot.get("ticker", "该标的")
    if snapshot.get("selected_preset_style"):
        return f"Buffett-Munger 式回答：{ticker} 是一篮子资产，能力圈不是理解每家公司，而是理解指数规则、成分集中度和长期驱动。"
    return f"Buffett-Munger 式回答：如果不能清楚说出 {ticker} 如何赚钱、客户为何选择它、竞争优势来自哪里，就先不要假装懂。"


def _answer_moat(snapshot: dict) -> str:
    style = str(snapshot.get("selected_preset_style") or "")
    if style:
        return "Buffett-Munger 式回答：ETF 没有单一公司的护城河，重点看底层成分是否普遍有护城河，以及指数是否把劣质公司也一起买进来。"
    return "Buffett-Munger 式回答：护城河必须能抵抗时间和竞争。只说公司很有名不够，要能解释定价权、转换成本或网络效应。"


def _answer_cash_flow(snapshot: dict) -> str:
    style = str(snapshot.get("selected_preset_style") or "")
    if style in {"科技成长", "成长"}:
        return "Buffett-Munger 式回答：成长 ETF 要看底层公司是否能把增长转成自由现金流，而不只是收入增速和叙事。"
    if style == "低波红利":
        return "Buffett-Munger 式回答：红利 ETF 要区分真实现金流支撑的股息，和因为股价下跌显得股息率很高的陷阱。"
    if style:
        return "Buffett-Munger 式回答：ETF 的现金流质量来自成分股整体质量，要关注盈利稳定性、资本回报率和行业周期性。"
    return "Buffett-Munger 式回答：好生意最终要体现在自由现金流和资本回报率上，否则增长可能只是昂贵的幻觉。"


def _answer_margin_of_safety(move_pct: float | None, risk_label: str | None) -> str:
    if move_pct is None:
        return "Buffett-Munger 式回答：没有价格和估值上下文，就谈不上安全边际。"
    if move_pct >= 5:
        return "Buffett-Munger 式回答：上涨后更要保守，先问价格是否已经把好消息都算进去了。"
    if move_pct <= -5:
        return "Buffett-Munger 式回答：下跌只有在内在价值未受损时才是机会，否则只是价值陷阱更便宜。"
    if risk_label == "波动偏高":
        return "Buffett-Munger 式回答：波动高时安全边际必须更厚，否则情绪会替你做决定。"
    return "Buffett-Munger 式回答：价格不极端时，不急着行动，先把内在价值区间和机会成本算清楚。"


def _answer_psychology(move_pct: float | None, trend: str | None) -> str:
    if move_pct is not None and move_pct >= 5 and trend in {"上升趋势", "短线偏强"}:
        return "Munger 式回答：最危险的是把上涨当成自己聪明。先写下反方理由，再决定是否值得承担追高风险。"
    if move_pct is not None and move_pct <= -5:
        return "Munger 式回答：下跌时也别急着装勇敢。先确认不是基本面变坏，再谈逆向。"
    return "Munger 式回答：没有强信号时，少动往往是优势。别为了显得勤奋而交易。"
